...
def dynamic_alocate_buffers(engine,input_shape):
    inputs = []
    outputs = []
    bindings = []
    stream = cuda.Stream()
    h,w = input_shape
    shapes = {'images':input_shape,
            'output1':(h//8,w//8),
            'output2':(h//16,w//16),
            'output3':(h//32,w//32)}
    for binding in engine:
        origin_shape = engine.get_binding_shape(binding)
        cur_shape = shapes[binding]
        origin_shape[2],origin_shape[3] = cur_shape
        size = trt.volume(origin_shape) * engine.max_batch_size
